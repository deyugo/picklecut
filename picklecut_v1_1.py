#!/usr/bin/env python3
"""
picklecut V1 — 匹克球比賽影片：標記回合、剪去死球時間、疊上常駐計分板，一個工具做完。
...
更新紀錄
    V1.1 2026-09-02
        ...
        · 字幕：輸入文字按鈕（或 Enter）即在目前時間點加入，燒錄在
          ...
          跨剪點自動重新對時
        · 多影片比賽：新增起始發球員編號（#1/#2）設定，接續上一支
          影片打到一半的一局；--join 指令無損串接各段成片（stream copy）
        ...
        · 比分紀錄：渲染時在成片旁寫 <名稱>.score.txt（局數結果與逐分
          紀錄：時間、局、發球者、勝方、比分、換發、★ highlight）；介面
          新增「匯出比分」鈕；--dry-run 也會印出
    V1 2026-08-31
        · 首個匹克球版本，改寫自桌球工具 ttcut V2.2
        · 計分引擎改為匹克球規則：傳統計分（發球得分制——只有發球方能得分；
          雙打每局從第二發球員開始，即「0-0-2」開局）與每球得分制
          （每回合都得分、由勝方發球），介面上可切換
        · 新增單打／雙打切換；雙打的發球員編號在計分板與介面上以
          一顆或兩顆強調色圓點顯示
        · 標記時即時顯示正式報分（例如 5-3-2）
        · 標記 JSON 升到 version 3，新增 sport / format.scoring /
          format.side 欄位；A/B 鍵的意義改為「該回合由 A/B 獲勝」，
          得分或換發由引擎判定
        · 「重發球」取代桌球的「擦網重發」（--cut-replays / --replay-tail）
        · 預設強調色改為匹克球黃綠 #BFD730
        · 計分之後的環節（剪輯、渲染、伺服器、命令列）沿用 ttcut V2.2 不變

用法:
    python3 picklecut_v1_1.py                                   ← 開介面（一般用這個）
    python3 picklecut_v1_1.py IMG_1496.tags.json IMG_1496.MOV   ← 命令列直接渲染
    python3 picklecut_v1_1.py tags.json video.MOV --quality max --dry-run
    python3 picklecut_v1_1.py --join g1.cut.mp4 g2.cut.mp4 -o match.mp4
...
PAD_L, PAD_B   = 64, 64
PANEL_W        = 532          # 比 ttcut 寬：得分欄右側多了發球圓點欄
ROW_H          = 54
...
def fold_full(events, fmt, start, first_server=0):
    ...
    回傳 dict:
        states  [(來源時間, gA, gB, a, b, server, serverNum), ...]
                給 ASS 計分板用，第一筆時間為 None
        snaps   與 events 等長；回合（point）事件給 dict，其餘為 None，
                給介面事件列表用。`scored` 為 False 代表該回合只換了發球權
                （side-out），沒有得分。
        cur     目前狀態，含下一球該誰發與發球員編號

    匹克球規則：
      side-out  只有發球方能得分。雙打每局從第二發球員開始（「0-0-2」開局）；
                發球方輸掉回合時由 #1 交給 #2，再輸就換發給對方（對方從 #1
                開始）。單打沒有發球員編號，輸掉回合直接換發。
                一局只能在發球方贏下回合時結束。
      rally     每回合都得分，且由該回合勝方發下一球（沒有發球員編號）。
    """
    ...
    def game_serve(gi):
        # 每局輪流先發；雙打 side-out 每局從第二發球員開始，其餘從 #1 開始。
        # 接續上一支影片打到一半的局，可只對開頭狀態覆寫發球員編號
    ...
    pending = False                        # 局末：先把比分留在畫面上，下一回合才歸零
    states = [(None, gA, gB, a, b, server, snum)]   # 開頭狀態，時間稍後補
    snaps = []

    def game_over():
        hi, lo = max(a, b), min(a, b)
        if mode == "capped" and hi >= cap:      # 先到 cap 者勝，不必贏兩分
            return True
        return hi >= T and hi - lo >= 2          # 標準：到達目標分且領先 2 分
    ...
    for e in events:
        ...
        if won:
            if a > b:
                ...
                gB += 1
            pending = True
            server, snum = game_serve(gi + 1)    # 下一局換另一方先發
        states.append((e["t"], gA, gB, a, b, server, snum))
...
def read_format(doc):
    """從 JSON 讀賽制與起始比分，缺欄位時退回預設值
    （雙打、side-out、11 分、贏兩分）。"""
    f = doc.get("format", {}) or {}
    ...
    start = dict(games=[int(g.get("A", 0)), int(g.get("B", 0))],
                 ...
                 serverNum=int(st.get("serverNum", 0) or 0))   # 0 = 新局預設
    return fmt, start
...
"""
    ...
    # ── 局數、得分與發球圓點（隨事件變動）：局數與得分同字級、靠底色分辨；
    # 發球方在右側欄位標一顆強調色圓點（雙打第二發球員標兩顆）
...
def plan(doc, opt):
    ...
    if not serves or not points:
        return dict(ok=False, reason="事件裡沒有發球或回合，無法剪接。",
    ...
    # 只有發球／回合／換局事件影響剪接；字幕、highlight 與校正
...
def summary_lines(plan_d, opt, video):
    ...
    rule = ("勝 2 分" if fmt["deuce"] == "standard"
            else f"封頂制（先到 {fmt['cap']} 分者勝）")
    bits = [f"{'雙打' if fmt.get('doubles', True) else '單打'}",
            f"{'傳統計分（side-out）' if fmt.get('scoring', 'sideout') == 'sideout' else '每球得分制'}",
            f"每局 {fmt['target']} 分", rule]
    if any(start["games"]):
    ...
    out.append(f"事件      {plan_d['points']} 回合 · {plan_d['serves']} 發球 · "
               f"{plan_d['serves'] - plan_d['points']} 次重發")
    out.append(f"參數      回合後留 {plan_d['tail']}s · 發球前留 {plan_d['lead']}s · "
...
def score_report(plan_d, names, video):
    ...
    out = [f"picklecut {VERSION} 比分紀錄",
           f"來源      {os.path.basename(video) if video else '—'}",
           f"A         {names[0]}",
           f"B         {names[1]}",
           f"賽制      {'雙打' if doubles else '單打'} · "
           f"{'傳統計分（side-out）' if fmt.get('scoring', 'sideout') == 'sideout' else '每球得分制'} · "
           f"每局 {fmt['target']} 分 · {rule}"]
    ...
    for i, e in enumerate(events):
        ...
        if e["type"] == "point" and s:
            ...
            rows.append((str(n), ts(e["t"]), str(s["gameNo"]),
                         ...
                         f"{s['a']}–{s['b']}",
                         ("換發" if s.get("scored") is False else
    ...
    out += ["", "逐分紀錄（時間為原始影片時間；★ = highlight" + ("；換發 = 該回合只換了發球權" if fmt.get("scoring", "sideout") == "sideout" else "") + "；發球欄 #1/#2 為雙打的發球員編號" + "）",
            "   #  時間        局  發球  勝方 比分"]
    for r in rows:
...
<title>picklecut __VERSION__ — 匹克球回合標記與剪輯</title>
...
<div class="shell">
  <header>
    ...
    <div class="ctl">A<input type="text" id="nameA" value="A 隊"></div>
    <div class="ctl">B<input type="text" id="nameB" value="B 隊"></div>
    <div class="ctl">首發<select id="firstServer"><option value="0">A</option><option value="1">B</option></select>
      <select id="firstServerNum" title="影片開始時的發球員編號（雙打傳統計分）。新局預設 #2；接續另一支影片打到一半的局時才需要改。">
        <option value="0">#2 · 新局</option>
        <option value="1">#1 · 接續</option>
        <option value="2">#2 · 接續</option>
      </select></div>
  ...
  <div class="setbar">
    ...
    <div class="grp"><span class="tag">賽制</span>
      ...
      <select id="scoring">
        <option value="sideout">傳統計分</option>
        <option value="rally">每球得分</option>
      </select></div>
    ...
    <div class="grp"><span class="tag">局末</span>
      <select id="deuce">
        <option value="standard">勝 2 分</option>
    ...
    <div class="grp"><span class="tag">計分板色</span>
      <input type="color" id="accent" value="#BFD730"
             title="得分數字、發球圓點與名字左側裝飾條共用這個顏色">
      <button class="btn" id="accentReset" title="回到預設黃綠色">重設</button></div>
  </div>
  ...
  <div class="stage">
    ...
    <div class="legend">
      ...
      <span><kbd>A</kbd>A 贏回合</span>
      <span><kbd>B</kbd>B 贏回合</span>
  ...
  <div class="rail">
    <div class="board">
      <div class="cards">
        ...
        <div class="card" id="cardB">
          <div class="who" id="whoB">B 隊</div>
          <div class="nums"><span class="g" id="gmB">0</span><span class="pts" id="ptsB">0</span><span class="dots" id="dotsB"></span></div>
          <div class="cap">局 · 分</div>
        </div>
      </div>
      <div class="boardmeta">
        <span>第 <b id="gameNo">1</b> 局 · 報分 <b id="callTxt">0–0–2</b><span id="ruleNote" class="rule"></span></span>
    ...
    <div class="cutout">
      ...
      <div class="pads">
        <div class="ctl">回合後留<input type="number" id="tailPad" value="1.0" step="0.1" min="0">s</div>
...
(() => {
  ...
  // 正式報分：發球方在前、接發方在後，雙打傳統計分再加發球員編號
  ...
  function paint(st) {
    ...
    if (!events.length) {
      stream.innerHTML = '<div class="streamempty">還沒有事件。<br>播放影片，在發球擊出的瞬間按 <b style="color:var(--ink)">S</b>，回合結束時按 <b style="color:var(--ink)">A</b> 或 <b style="color:var(--ink)">B</b> 標記勝方——得分或換發由系統自動判定。</div>';
    } else {
      ...
      events.forEach((e, i) => {
        ...
        else { cls = 'point'; label = nm[e.winner === 'A' ? 0 : 1] + ' 贏回合'; }
        prevServe = e.type === 'serve' ||
          (['caption','highlight','adjust'].includes(e.type) && prevServe);
        const sc = s ? (s.won ? `<em>${s.gA}–${s.gB} 局</em>`
                              : s.scored === false ? '換發'
...
def main():
    ...
    p = argparse.ArgumentParser(
        description=f"picklecut {VERSION} — 匹克球標記與剪輯。不給參數就開介面。")
    ...
    p.add_argument("--tail", type=float, default=None, help="回合後保留秒數（預設讀 JSON）")
    ...
    p.add_argument("--accent", default=None,
                   help=f"計分板強調色（得分數字、發球圓點與側邊裝飾條），例如 \"#BFD730\"。"
                        f"預設 {DEFAULT_ACCENT}")
